...
import { createFaq, updateFaq, deleteFaq, getAdminFaqs } from '@services/api';
import { PencilSquareIcon, TrashIcon } from '@heroicons/react/24/solid';
import FaqModal from '@components/modal/FaqModal';
import ConfirmationModal from '@components/modal/ConfirmationModal';

const AdminFaqsMain = ({ initialFaqs }) => {
  ...
  const refetchFaqs = async () => {
    try {
      const updatedFaqs = await getAdminFaqs();
      setFaqs(updatedFaqs);
    } catch (error) {
      console.error('Failed to refetch FAQs:', error);
    }
  };

  const addMutation = useMutation(createFaq, {
    onSuccess: () => {
      refetchFaqs(); // Manually refetch FAQs
      setModalOpen(false);
    },
  });
